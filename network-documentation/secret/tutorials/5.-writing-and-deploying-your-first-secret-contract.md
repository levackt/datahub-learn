...
// Query again to see that it incremented
response = await client.queryContractSmart(contractAddress, { "get_count" : {}});
console.log('Count: ', response.count);
```
...
```
----------------------------------------------------------------
Copy `transactionHash` from the output of either the contract upload transaction, or the contract execution to update the counter, and replace `TRANSACTION_HASH` in the link below to see the transaction in Secret Explorer!
https://explorer.secrettestnet.io/transactions/TRANSACTION_HASH
